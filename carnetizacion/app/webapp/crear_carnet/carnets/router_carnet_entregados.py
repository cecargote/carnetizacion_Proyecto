...
from db.repository.carnet_activo import lista_entregados
...
@router.get("/carnets/entregados")
async def carnet_entregados(request: Request, db: Session = Depends(get_db)):
    print("estoy en carnet Entregados")
    try:
        token = request.cookies.get("access_token")
        scheme, param = get_authorization_scheme_param(token)
        
        carnets = lista_entregados(db=db)
        ...
        response = templates.TemplateResponse(
            "general_pages/carnets/carnets_entregados.html", {"request": request, "carnets": carnets, "persons": persons, "motivos":motivos, 'page_obj': page_obj}
        )
        try:
